...
grid = np.zeros(1000000)
grid = grid.reshape((1000,1000))

def parse(string:str):
    string_list = string.split(" ")
    if string_list[0] == "toggle":
        instruction = "toggle"
        start_pair = string_list[1].split(",")
    else:
        instruction = string_list[1]
        start_pair = string_list[2].split(",")

    end_pair = string_list[-1].split(",")

    start_x = int(start_pair[0])
    start_y = int(start_pair[1])

    end_x = int(end_pair[0]) + 1
    end_y = int(end_pair[1]) + 1

    return instruction, start_x, start_y, end_x, end_y

def execute_inst(instruction, start_x, start_y, end_x, end_y):
    if instruction == "on":
        grid[start_y:end_y, start_x:end_x] += 1

    elif instruction == "off":
        f = np.vectorize(lambda x: x-1 if x!=0 else x)
        grid[start_y:end_y, start_x:end_x] =  f(grid[start_y:end_y, start_x:end_x])

    elif instruction == "toggle":
        grid[start_y:end_y, start_x:end_x] += 2

def main_func(string:str):
    string_list = string.split("\n")
    for line in string_list:
        if line:
            instruction, start_x, start_y, end_x, end_y = parse(line)
            execute_inst(instruction, start_x, start_y, end_x, end_y)
...
input_str = """turn on 489,959 through 759,964
